...
M = data.frame(
  Nreserve       = c(0, 0.5, 2, 5)/100,
  Photosynthesis = c(0, 0.00, 1.0, 1.0) 
)

make_plot = function() {
  ggplot(M, aes(Nreserve, Photosynthesis)) +
    geom_point(data=M[c(1,4),], size=2, colour=green) +
    geom_line(colour=green) +
    scale_x_continuous(breaks=M$Nreserve, labels = c(0, 0.005, 0.02, 0.05)) +
    labs(x="Plant nitrogen concentration", y="Relative\nphotosynthesis") +
    theme1
}
...
write_figure(output_folder, "N-response.png", make_plot(), W, H)
