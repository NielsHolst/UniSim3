...
M = data.frame(
  Temperature = c(-2,5,15,24),
  Scaling     = c(0, 1, 1, 0) 
)

make_plot = function() {
  ggplot(M, aes(Temperature, Scaling)) +
    geom_line(colour=red) +
    scale_x_continuous(breaks=M$Temperature) +
    labs(x="Temperature", y="Relative\ngrowth demand") +
    theme1
}
...
write_figure(output_folder, "T-response.png", make_plot(), W, H)
