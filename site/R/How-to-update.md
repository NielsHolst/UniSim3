...
* public_html
  * download *with a copy of local `site/download` files + installation files + etc.*
  * fonts *a true copy of local `site/fonts`*
  * input *a true copy of local `site/input`*
    * demo 
    * models 
      * aphid 
    * scripts 
  * media *a true copy of local `site/media`*
    * demo 
    * models
      * aphid 
...
* UniSim3
  * input *with all original scripts &minus; to be copied `site/input` folder for testing*
    * demo
    * models *with scripts for documentation*
      * aphid *with scripts for documentation*
    * scripts *with standard scripts*
  * output
    * batch_output *with outputs generated by batch command &minus; to be copied to `site/media` folder for testing*
      * demo *with PNG files produced my demo boxscripts*
      * models
        * aphid *with PNG files produced my aphid boxscripts and R scripts*
  * site *the complete local site, to be copied to the web site*
    * HTML files *generated by `site/R/update_html.R`*
    * download *with original (huge) sim output files needed for documentation*
    * fonts *with original font files*
    * input *copied from `../input`*
    * media *copied from `../media`*
    * R *with scripts to generate HTML files*
    * snippets
      * HTML files *with header, footer and righthand callouts*
      * plugins *with code generated by `help HTML` command*

Use **FileZilla** to transfer files to **NordicWay** host. When transferring the `UniSim3/site` to `public_html`, it is not necessesary to include `site/R` and `site/snippets`.

Finally, run [https://www.deadlinkchecker.com](https://www.deadlinkchecker.com/multisite.asp).
...
## Update HTML pages locally

1. Make a backup of the `UniSim3/site` folder to `UniSim3/site-yyyy-mm-dd`.
2. In Universal Simulator run `batch demo` and paste to R. Any missing PNG figures will be reported in R.
3. In Universal Simulator run `batch models` and paste to R. Any missing PNG figures will be reported in R.
4. Copy the PNG files from `UniSim3/output/batch_output` to `UniSim3/site/media`. Maintain the sub-folder structure.
5. In Universal Simulator run `help HTML` to update documentation snippets.
6. Run the  `update-html.R` script to regenerate all the HTML files. The new files ends up in the `UniSim3/site_new` folder.
7. Run `replace_site_input.bat` to clean the `site/input` folder and then fill it with a copy from `Unisim3/input`.
8. Copy the HTML files from `UniSim3/site_new` to `UniSim3/site` and check that it works.
