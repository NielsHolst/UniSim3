...
M = data.frame(
  Salinity = c(0,16,25,30),
  Scaling  = c(0,0.5,1,1) 
)

make_plot = function() {
  ggplot(M, aes(Salinity, Scaling)) +
    geom_line(colour=blue) +
    scale_x_continuous(breaks=M$Salinity[1:3]) +
    labs(x="Salinity", y="Relative\ngrowth demand") +
    theme1
}
...
write_figure(output_folder, "sal-response.png", make_plot(), W, H)
