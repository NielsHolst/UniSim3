...
make_plot = function(size1, size2, colours) {
  alpha = if (all(colours==bw)) 1 else 0.3
  ggarrange(
...
dev.off()
print(paste("Figure written to", getwd(), "fig-2-bw.png"))

# Black and white for journal
cairo_ps("fig-2-bw.eps", width=mm(W), height=mm(H))
print(make_plot(0.3,1,bw))
dev.off()
print(paste("Figure written to", getwd(), "fig-2-bw.eps"))

# Hook for batch command
figures = function(df) {
  Pages = list(Page = list(Grob=make_plot(0.3,1,colours), Width=mm(W), Height=mm(H)))
}
