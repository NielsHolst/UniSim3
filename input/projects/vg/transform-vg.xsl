...
<xsl:template name="extract-setpoints">
  ...
  <port name="initialSignal" value="0">  
    <xsl:if test="count($setpoint-index)>0 and string-length($encodeClassName)=0">
      <xsl:attribute name="value">
      ...
      <port name="signalInside">
        <xsl:attribute name="value">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:value-of select="'./encode[value]'"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:attribute>
      </port>
    ...
    <box name="elementary">
      ...
      <box class="PrioritySignal" name="screenLayer1">
        <xsl:call-template name="extract-setpoints">
          ...
          <xsl:with-param name="encodeClassName" select="'ScreenCombinationCode'"/>
        </xsl:call-template>
      </box>
